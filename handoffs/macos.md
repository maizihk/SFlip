...
- 环境与后续：本机缺少 Xcode / MSBuild；PR 后通过双端现有 CI 验证测试、构建、签名及打包，结果待回填。未执行真实硬件、网络探测、权限或系统设置操作；既有实机待验项保持。
...
- 静态确认：303 个界面/错误模板都有英文，无缺失调用键；新目录占位符匹配；剩余源码汉字只为兼容旧名称正则、稳定 UDP 日志/操作 tag，以及展示时再本地化的原状态模板。临时库存文件已删除。`git diff --check` 通过。
...
- 此次仅修改 `SettingsWindowController.swift`、Mac 清单和本交接文件。代码 `2f121e4` 通过 CI [34752734642](https://github.com/maizihk/SFlip/actions/runs/34752734642)：297 项 XCTest、Debug/Release、严格签名和产物验证。测试 DMG：`macOS/outputs/language-row-2f121e4/package/SFlip-macOS-arm64.dmg`；artifact `10315872798`，下载 ZIP SHA-256 `9cb7f74e62f13b17ddc0d96564dd8783b04fe8ce7255105927472c42e5c8f263` 已校验。语言行 GUI 待用户复验，未执行真实硬件流程。
...
## 当前任务：README 仓库更名遗漏修正（2026-09-06）

- 分支：`codex/macos-readme-repo-rename`；基线：`e300a94`，已合并 PR #84。
- 原因：GitHub 仓库改名后，README 仍使用旧仓库 URL、克隆目录及“仓库地址保留原名”的说明。
- 修改：徽章、仓库/发布/PR 链接、克隆命令和说明统一指向 SFlip，明确当前 ZIP 名称；已通过 GitHub API 核对 v2.2.0 仍使用旧附件名，下载文件名与内部工程路径保持真实值。
- 验证：文档链接/路径与旧名称残留分类检查、`git diff --check`；纯 README 和交接记录修改，不重复平台构建或执行硬件流程，平台 CI 路径过滤不触发。
...
- 外观补充：新增 AppKit Retina 背景生成脚本、dmgbuild 布局和固定依赖清单；首次打包在构建目录建立 Python 虚拟环境。固定图标坐标、卷图标、箭头与安装提示，文本说明隐藏以保持双图标界面。本机 Finder 已查看，窗口为状态栏及路径栏预留高度；美化后 272 项测试仍全过。
...
- 范围：只修改共享 `README.md` 和本交接记录，不涉及平台功能清单、代码、协议、版本或发布包。
- 调整：下载/安装前置；更正先启用 DDC 项再读取的顺序，补齐检测和确认对端身份；解释 USB 物理转接边界、媒体键权限、发布包与主线差异；构建要求以工程和 CI 配置为准。
- 验证：发布附件名称与当前 v2.2.0 Release 核对；相对链接及 Markdown 代码块检查通过，按钮名称、权限和工具链要求与源码对应，`git diff --check` 通过。
- 首次使用补充：新增 `docs/INPUT_SOURCES.md`，说明 0x60 与输入值、目标接口方向、Windows/macOS 手动查询、十六进制换算和失败边界；README 的 USB/协同步骤均链接教程。依据工具官方文档和源码核验，仅提供操作指引，未安装工具或实测硬件。
- 纯文档变更不执行硬件操作或重复平台构建；现有工作流路径过滤不覆盖根 README / handoffs，本次 PR 不预期触发平台 CI。
