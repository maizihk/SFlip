...
- 新模拟回归直接使用生产探测、保存发布和缓存同步 helper，覆盖慢响应、过期、保存故障/同值恢复、页面隔离和 NFC；本机 x64 Release 因缺少 64 位 MSBuild 无法执行，最终结果待 PR CI。
- 首轮 CI 编译完成但恢复断言失败，根因是同值重试 fixture 默认关闭却断言开启，并非生产保存异常；已明确开启意愿、拆分恢复断言并新增关闭配置不自动开启回归，结果待新 CI。
