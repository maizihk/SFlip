...
- 自动验证和 x64 Release/CI 结果由本次交付记录补齐；现有模拟读取测试覆盖批读取、单台失败隔离、取消和零写入。未知/混合、高对比度、DPI、窄窗口及公共读取仍需实机视觉验收；未启动真实应用或执行硬件、网络及系统设置动作。
...
- 分支 `codex/windows-linked-display-controls`，基线 `1634bb7`；不改协议、schema 或版本。
- 联动页固定显示三个统一控制行，功能/托盘混合偏好显示“部分开启”；逐台卡隐藏控制网格，保留显示器状态、读取、绑定和删除操作。
- 主开关保存只切换模式，不批改功能/托盘偏好。统一功能操作包含离线目录，关功能清托盘；托盘只作用功能已开启项目，无可用项目时禁用。公共滑杆继续使用既有在线唯一绑定投影和DDC安全过滤。
- 捕获逐台控件使用已保存的联动状态：off转on捕获现有单台值，on转off忽略隐藏控件，恢复当前偏好而非历史快照。初始化订阅前赋值，用户操作保存成功后延迟重建，失败恢复已保存配置。
- 新增生产模拟回归覆盖全部关闭、混合、离线偏好、模式双向切换、隐藏网格、保存失败与非控制VCP保护，并通过fake service验证在线目标过滤及不支持后端零写入。
- 提交 `5699eaa` 已通过 [Windows CI run 34682848739](https://github.com/maizihk/SFlip/actions/runs/34682848739) / job `103524555132`：517 项原生检查、61 项安装器检查、x64 Release 和绿色版上传全部成功；附件 ID `10294268194`。PR [#101](https://github.com/maizihk/SFlip/pull/101) 目标为 `codex/windows-docs-refresh`。
- 本机仍缺少 64 位 MSBuild；设置页宽度、混合提示及模式转换视觉和实机验收仍待完成。未执行真实网络、USB、DDC、输入源、唤醒或权限操作。

## 当前任务：W-043 用户文档精简（2026-09-12）

- 分支 `codex/windows-docs-refresh`，基线 `6560391`；根 README 和 Windows README 精简为安装、使用、常见问题和构建入口，保留必要依赖与升级配置说明。
- `docs/releases/v2.3.0.md`、`v2.3.1.md`、`v2.3.2.md` 内容已通过 GitHub REST 更新现有 Release body；版本和附件未变。
- 本地 Markdown 链接及 `git diff --check` 通过。本次纯文档修改未新增平台构建；前置代码 `6560391` 双端 CI 已成功。
- 未新增 GUI 或硬件验收，既有设置页和实机待验项目保持原状态。

## 当前任务：W-042 连接与配置保存恢复一致性（2026-09-12）

- 分支 `codex/windows-product-flow-review`，基线 `32d8299`；只修改 Windows 生产路径、模拟回归和本平台文档，不改协议、schema、版本或硬件动作。
- 周期探测原先每 2 秒覆盖仍有 10 秒有效期的单一事件，合法慢响应会被拒绝；现在未过期事件继续等待，成功后下一周期发送新事件，超时后才替换。
- 保存失败后 UI 标记需重新持久化，同值重试不再跳过；完整保存返回成功后才解除 runtime/UI 安全标记，失败仍保留旧数据并阻断连接及硬件。
- 后台路由变化只轻量同步缓存到匹配 profileID、地址、端口和 NFC 配对码的 UI 副本；先捕获草稿并保留名称、启用与无关编辑。地址、端口或码变化清旧缓存，避免其他页保存回滚首次连接结果。
- 新模拟回归直接使用生产探测、保存发布和缓存同步 helper，覆盖慢响应、过期、保存故障/同值恢复、页面隔离和 NFC；本机 x64 Release 因缺少 64 位 MSBuild 无法执行，完整验证由 CI 完成。
- 首轮 CI 编译完成但恢复断言失败，根因是同值重试 fixture 默认关闭却断言开启，并非生产保存异常；已明确开启意愿、拆分恢复断言并新增关闭配置不自动开启回归，后续 CI 全部通过。
- 提交 `6560391` 已通过 [Windows CI run 34680995062](https://github.com/maizihk/SFlip/actions/runs/34680995062) / job `103519478792`：503 项原生检查、61 项安装器检查和 x64 Release 构建全部成功。
- 未执行真实网络、USB、DDC、输入源、唤醒或系统权限操作；设置页失败恢复、后台连接后的编辑及多配置视觉状态仍待实机验收。

## 当前任务：W-041 Windows 设置页文案精简（2026-09-11）

- 分支 `codex/windows-settings-copy-cleanup`，基线 `2371f44`；只修改 Windows 用户可见文案和 Windows 文档，不改协议、schema、版本或控件行为。
- 协同页移除常驻的权限步骤和零硬件说明；诊断页移除预览实现与复制一致性说明。按钮、只读预览、刷新、复制和诊断导出逻辑保持原样。
- 权限检查进度与结果及前置失败只保留用户可执行的信息，不再显示 UDP 等实现术语；真实配置、保存、地址、端口、权限和网络错误继续明确提示。
- 附加布局按用户要求将连接状态行放到协同配置之后的页面底部；权限和检测反馈保留既有窗口底部区域，按钮、配置编辑和回调逻辑不变。
- 协同页连接、网络检查和手动检测共用唯一状态行；连接成功统一为“已和对端（配置名）建立连接”，不再另显示“已连接”或网络检查成功提示。
- 未执行网络、USB、DDC、输入源、唤醒或系统权限操作；x64 Release 与完整回归结果待补。

## 当前任务：W-040 USB 与协同设置独立保存（2026-09-12）

- 分支 `codex/windows-independent-settings`，基于尚未合并主线的 PR #94 提交 `b411f20`；不改协议、schema 或版本。
- 设置保存按 `Collaboration`、`Usb` 和 `None` 范围分别捕获、校验和合并，非当前页面保留已保存值，协同开关不再因 USB 状态失败或跳页。
- USB 联动可引用仍存在但暂时关闭或正在补充的协同配置并保留选择与意愿；运行时仍不发送协同唤醒。缺失引用仍安全拒绝/清理；用户显式新开启联动或更换目标时才要求目标完整。
- 删除被联动引用的协同配置会在同次保存关闭联动并清空引用，UI 以 loading guard 避免 Toggle 触发嵌套保存。
- 生产合并 helper 的模拟回归覆盖协同/USB 双向隔离、disabled 且不完整引用保存加载、已有联动不阻塞 USB 保存、删除清理及零网络唤醒。未执行真实 USB、DDC、输入源、唤醒或网络操作；本机缺少 64 位 MSBuild，完整构建和测试待 CI。
