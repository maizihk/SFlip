...
if [[ $# -ne 2 ]]; then
    echo "Usage: $0 <SFlip.app> <output.dmg>" >&2
    exit 2
fi
APP_PATH="$1"
DMG_PATH="$2"
...
WORK_DIR="$(/usr/bin/mktemp -d /private/tmp/SFlip-dmg.XXXXXX)"
...
/usr/bin/ditto --norsrc "$APP_PATH" "$CONTENTS/SFlip.app"
/bin/ln -s /Applications "$CONTENTS/Applications"
cat > "$CONTENTS/安装说明.txt" <<'INSTRUCTIONS'
安装 SFlip

1. 将 SFlip.app 拖到旁边的 Applications（应用程序）文件夹。
...
"$TOOLS_DIR/bin/dmgbuild" -s "$PROJECT_DIR/scripts/dmg/settings.py" \
    ...
    SFlip "$WORK_DIR/SFlip.dmg"
/usr/bin/hdiutil verify "$WORK_DIR/SFlip.dmg" >/dev/null
/usr/bin/hdiutil attach "$WORK_DIR/SFlip.dmg" -readonly -nobrowse \
    -noautoopen -mountpoint "$MOUNT_POINT" >/dev/null
MOUNTED=true
[[ -d "$MOUNT_POINT/SFlip.app" ]]
[[ -L "$MOUNT_POINT/Applications" ]]
[[ "$(/usr/bin/readlink "$MOUNT_POINT/Applications")" == /Applications ]]
[[ -f "$MOUNT_POINT/安装说明.txt" ]]
[[ -f "$MOUNT_POINT/.DS_Store" ]]
/usr/bin/codesign --verify --deep --strict "$MOUNT_POINT/SFlip.app"
# Verify the copy that Finder will install, without touching /Applications.
/usr/bin/ditto --norsrc "$MOUNT_POINT/SFlip.app" "$WORK_DIR/installed/SFlip.app"
/usr/bin/codesign --verify --deep --strict "$WORK_DIR/installed/SFlip.app"
/usr/bin/hdiutil detach "$MOUNT_POINT" >/dev/null
MOUNTED=false
/bin/mv -f "$WORK_DIR/SFlip.dmg" "$DMG_PATH"
echo "$DMG_PATH"
